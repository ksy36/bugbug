...
from bugbug.github_dbs_config import get_config
...
# Register all databases from the config
for key, value in get_config().items():
    db.register(value["path"], value["url"], value["version"])
    DATABASES[key] = value["path"]
